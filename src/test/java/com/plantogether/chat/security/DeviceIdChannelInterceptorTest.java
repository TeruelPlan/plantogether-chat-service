 ...
 *
 * Acceptance criteria:
 * - STOMP CONNECT with valid X-Device-Id UUID → principal set on accessor
 * - STOMP CONNECT without X-Device-Id → principal remains null (broker rejects unauthenticated)
 * - STOMP CONNECT with invalid UUID format → principal remains null
 * - Non-CONNECT frames are not modified
 *
 * TDD RED PHASE: Remove @Disabled once verified passing.
 * These tests exercise existing implementation — expected to pass after enabling.
 */
